...
module.exports = {
	name: 'pgen', // command name at execute (may be different from the file name)
	...
	execute(message, args) {
        ...
        try {
            message.client.channels.cache.get(config.pgenChannel).id; // get the gen channel id (for testing the config)
        } catch (error) {
            ...
            if (config.command.error_message === true) {
                return message.channel.send(
                    ...
                    .setTitle('Error!')
                    .setDescription('I did not find the gen channel.')
        ...
        if (message.channel.id === config.pgenChannel) {
            ...
            if (generated.has(message.author.id)) {
                ...
                const filePath = `${__dirname}/../pstock/${args[0]}.txt`; // path for the specified service
                    ...
                    if (!error) {
                        ...
                        if (position === -1) {
                            return message.channel.send(
                                ...
                                .setColor(config.color.red)
                                .setTitle('No Stock!')
                                .setDescription(`I did not find \`${args[0]}\` service in my stock!`)
                                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                .setTimestamp()
                            );
                        };
                        ...
                        if (position !== -1) {
                            ...
                            fs.writeFile(filePath, data, function (error) {
                                message.channel.send(
                                    ...
                                    .setColor(config.color.green)
                                    .setTitle('Service Generated!')
                                    .setDescription(`Check your dms, ${message.author}! **If you didn't receive the message, unlock your dms!**`)
                                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                    .setTimestamp()
                                );
            ...
            message.channel.send(
                ...
                .setDescription(`You cannot use the \`pgen\` command in this channel! Only in <#${config.pgenChannel}>!`)
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                .setTimestamp()
            );
        };
